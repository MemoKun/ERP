...
class CreateShopsTable extends Migration {
	...
	public function up()
	{
		Schema::create('shops', function(Blueprint $table)
		{
			...
			$table->integer('warehouse_id')->unsigned()->comment('默认仓库');
			$table->string('shop_account')->default('')->comment('店铺账号');
			$table->string('shop_passwd')->default('')->comment('店铺密码');
			$table->decimal('rebate', 10)->default(0.00)->comment('返点（%）');
			...
			$table->decimal('gross_profit_rate', 10)->default(0.00)->comment('毛利率（%）');
			$table->integer('platform_id')->default(0)->comment('平台类型');
			$table->boolean('is_waybill')->default(1)->comment('电子面单');
