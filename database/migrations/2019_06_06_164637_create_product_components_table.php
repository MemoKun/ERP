...
class CreateProductComponentsTable extends Migration {
	...
	public function up()
	{
		Schema::create('product_components', function(Blueprint $table)
		{
			...
			$table->string('component_name')->default('')->comment('子件名称');
			$table->decimal('tb_price', 10)->default(0.00)->comment('淘宝价格');
			$table->decimal('cost', 10)->default(0.00)->comment('成本价格');
			$table->decimal('price', 10)->default(0.00)->comment('售价');
			$table->decimal('highest_price', 10)->default(0.00)->comment('最高售价');
			$table->decimal('lowest_price', 10)->default(0.00)->comment('最低售价');
			$table->decimal('warehouse_cost', 10)->default(0.00)->comment('仓库成本');
			$table->decimal('assembly_price', 10)->default(0.00)->comment('装配价格');
			$table->decimal('discount', 10)->default(0.00)->comment('折扣');
			$table->decimal('commission', 10)->default(0.00)->comment('佣金点');
			$table->boolean('is_common')->default(0)->comment('是否通用子件');
			$table->integer('package_quantity')->default(0)->comment('包件数量');
			$table->decimal('package_costs', 10)->default(0.00)->comment('打包费用');
			...
			$table->boolean('distribution_method_id')->comment('配送类别');
			...
			$table->float('longness', 10)->default(0.00)->comment('长度（mm）');
			$table->float('width', 10)->default(0.00)->comment('宽度（mm）');
			$table->float('height', 10)->default(0.00)->comment('高度（mm）');
			$table->float('volume', 10)->default(0.00)->comment('体积(m²)');
			$table->float('weight', 10)->default(0.00)->comment('重量');
			$table->string('remark')->default('')->comment('备注');
			$table->boolean('finished_pro')->default(0)->comment('是否成品 0=不是 1=是');
			$table->boolean('is_stop_pro')->default(0)->comment('是否停产 0 不是 1 是');
			$table->timestamps();
		});
	}
