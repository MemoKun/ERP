...
class CustomerServiceDepartmentsController extends Controller
{
    ...
    public function orderCenter(CustomerServiceDepartmentRequest $request)
    {
        ...
        //$created_at = $request->input('created_at');
        $logistics_id = $request->input('logistics_id');
        //$stockout_at = $request->input('stockout_at');
        //$payment_date = $request->input('payment_date');
        $order_status = $request->input('order_status');
        $freight_types_id = $request->input('freight_types_id');
        $seller_flag = $request->input('seller_flag');
        $logistics_sn = $request->input('logistics_sn');
        $seller_remark = $request->input('seller_remark');
        ...
        ->where('logistics_id', 'like', '%'.$logistics_id.'%')
        //->where('stockout_at', 'like', '%'.$stockout_at.'%')
        //->whereBetween('payment_date', [$payment_date[0], $payment_date[1]])
        ->where('order_status', 'like', '%'.$order_status.'%')
        ->where('freight_types_id', 'like', '%'.$freight_types_id.'%')
        ->where('seller_flag', 'like', '%'.$seller_flag.'%')
        ->where('logistics_sn', 'like', '%'.$logistics_sn.'%')
        ->where('seller_remark', 'like', '%'.$seller_remark.'%')
        ->where('warehouses_id', 'like', '%'.$warehouses_id.'%')
        //->whereBetween('audit_at', [$audit_at[0], $audit_at[1]])
        ->where('auditor_id', 'like', '%'.$auditor_id.'%')
        ->orderBy('created_at', 'desc');
