...
use App\Transformers\RedPackageTransformer;
...
use App\Http\Requests\Api\RedPackageRequest;
...
class RedPackageController extends Controller
{
    ...
    const TRANSFORMER = RedPackageTransformer::class;
    const MODEL = RedPackage::class;
    const PerPage = 8;
     ...
     * })
     */
    public function index(RedPackageRequest $request)
    {
     ...
     * })
     */
     public function store(RedPackageRequest $request)
     {
     ...
     * })
     */
    public function update(RedPackageRequest $productRequest,
                           RedPackageComponentRequest $productComponentRequest,
                           RedPackage $product)
    {
     ...
     * })
     */
    public function destroy(RedPackage $product)
    {
        DB::transaction(function () use ($product) {
            ...
            //删除产品
            $delRedPackage = $product->delete();

            if ($delCom === false || $delComp === false || $delRedPackage === false) {
    ...
    public function destroybyIds(DestroyRequest $request)
    {
        ...
        DB::transaction(function () use ($ids) {
            ...
            $product = RedPackage::whereIn('id',$ids)->with('productComponents')->get();
            //删除子件 还有子件的库存 和 订单没有删除
            ...
            //删除产品
            $delRedPackage = RedPackage::destroy($ids);
