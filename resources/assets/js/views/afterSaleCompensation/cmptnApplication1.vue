      ...
      <el-tab-pane label="订单列表" name="0">
        <div>
          <div class="searchBox">
            <span>
              <label>买家昵称</label>
              <el-input v-model="searchBox.cmptn_buyerNickname" clearable></el-input>
            </span>
            <span>
              <label>买家姓名</label>
              <el-input v-model="searchBox.cmptn_buyerName" clearable></el-input>
            </span>
            <span>
              <label>买家电话</label>
              <el-input v-model="searchBox.cmptn_buyerPhone" clearable></el-input>
            </span>
            <span v-if="filterBox">
              <label>买家地址</label>
              <el-input v-model="searchBox.cmptn_buyerAddress" clearable></el-input>
            </span>
            <span v-else>
          ...
          <div class="searchBox" v-show="filterBox">
            <span>
              <label>业务员</label>
              <el-input v-model="searchBox.cmptn_businessPersonnel" clearable></el-input>
            </span>
            <span>
              <label>赔偿方向</label>
            ...
            <span>
              ...
              <el-select v-model="searchBox.cmptn_responsibleParty" clearable placeholder="请选择">
                <el-option v-for="item in searchBox.cmptn_responsiblePartys" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </span>
            <span>
              <label>责任人</label>
              <el-input v-model="searchBox.cmptn_responsiblePerson" clearable></el-input>
            </span>
          </div>
          <div class="searchBox" v-show="filterBox">
            <span>
              ...
              <el-select v-model="searchBox.cmptn_logisticsCompany" clearable placeholder="请选择">
                <el-option v-for="item in searchBox.cmptn_logisticsCompanys" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </span>
            <span>
              <label>物流单号</label>
              <el-input v-model="searchBox.cmptn_logisticsTrackingNumber"></el-input>
            </span>
            <span>
              <label>所属店铺</label>
              <el-select v-model="searchBox.cmptn_orderShop" clearable placeholder="请选择">
                <el-option v-for="item in searchBox.cmptn_orderShops" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </span>
          </div>
        ...
        <el-tabs v-model="leftTopActiveName" @tab-click="leftHandleClick" style="height: 400px;">
          <el-tab-pane label="未审核" name="0">
            <el-table :data="cmptnOrderListData" fit v-loading="loading" height="350">
              <!--选择列-->
          ...
          <el-tab-pane label="已审核" name="1">
            <el-table :data="cmptn_alreadyHandle" fit height="350">
              <el-table-column type="selection" width="95" align="center" :checked="checkboxInit">
              </el-table-column>
              <el-table-column v-for="item in cmptnOrderListHead" :label="item.label" align="center" :width="item.width" :key="item.label">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    outerHandleClick() {},
    fetchData() {
      this.$fetch(this.urls.aftercompensation).then(
        res => {
          this.loading = false;
    ...
    /*页码*/
    handlePagChg(page) {
      this.$fetch(this.urls.aftercompensation + "?page=" + page, {
        include:
    ...
    lockOrder() {
      if (this.newOpt[3].nClick) {
        ...
        this.$put(
          this.urls.aftercompensation + "/" + id + "/lockorunlock"
        ).then(
          () => {
            this.newOpt[1].nClick = false;
            this.newOpt[2].nClick = false;
