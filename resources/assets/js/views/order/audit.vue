...
export default {
  ...
  methods: {
    ...
    /*页码*/
    handlePagChg(page){
      this.$fetch(this.urls.financialdepts+'?page='+page,{include:'shop,logistic,freightType,distribution,distributionMethod,distributionType,takeDeliveryGoodsWay,customerType,paymentMethod,warehouses,orderItems,businessPersonnel,locker,paymentDetails'})
        .then(res=>{
    ...
    handleReject(){
      if(this.newOpt[0].nClick){
        ...
        this.$put(this.urls.financialdepts+'/'+id+'/reject')
          .then(()=>{
            this.refresh();
            this.$message({
              message: '驳回成功',
    ...
    handleFinancialAudit(){
      if(this.newOpt[1].nClick){
        ...
        let id = this.checkboxId?this.checkboxId:this.curRowId;
        this.$put(this.urls.financialdepts+'/'+id+'/financialaudit')
          .then(()=>{
            this.refresh();
            this.$message({
              message: '财务审核成功',
    ...
    handleUnFinancialAudit(){
      if(this.newOpt[2].nClick){
        ...
        let id = this.checkboxId?this.checkboxId:this.curRowId;
        this.$put(this.urls.financialdepts+'/'+id+'/unfinancialaudit')
          .then(()=>{
            this.refresh();
            this.$message({
              message: '退回财务审核成功',
    ...
    payDtlRCName({row,rowIndex}){row.index = rowIndex;},
    payDtlRClick(row){
      this.payDtlIndex = `index${row.index}`;
      console.log(row);
    },
    updatePayDtl(row){
      this.$patch(this.urls.paymentdetails+'/'+row.id,row)
        .then(()=>{
          this.refresh();
          this.$message({
            message: '支付明细更新成功',
    ...
    addPayDtlConfirm(){
      ...
      this.$post(this.urls.paymentdetails,submitData)
        .then(()=>{
          this.refresh();
          this.$message({
            message: '新增支付明细成功',
