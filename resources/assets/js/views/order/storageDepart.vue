<template>
    <div>
        <el-tabs v-model="activeName" @tab-click="outerHandleClick">
                ...
                <div>
                    ...
                    <div class="searchBox" v-show="filterBox">
                        <span>
                                ...
                                <el-select v-model="searchBox.order_type" clearable placeholder="请选择">
                                    <el-option
                                            v-for="item in searchBox.orderType"
                                            :key="item.value"
                                            :label="item.label"
                                            :value="item.value">
                                    </el-option>
                        ...
                        <span>
                                <label>所属仓库</label>
                                <el-select v-model="searchBox.order_warehouse" clearable placeholder="请选择">
                                    <el-option
                                            v-for="item in searchBox.ordertWarehouse"
                                            :key="item.value"
                                            :label="item.label"
                                            :value="item.value">
                                    </el-option>
                    ...
                    <div class="searchBox" v-show="filterBox">
                        ...
                        <span>
                                <label>财审日期</label>
                                 <el-date-picker
                                         v-model="searchBox.order_financeInves"
                                         type="daterange"
                                         range-separator="至"
                                         start-placeholder="开始日期"
                                         end-placeholder="结束日期">
                        ...
                        <span>
                                <label>打印时间</label>
                                <el-date-picker
                                        v-model="searchBox.order_printDate"
                                        type="daterange"
                                        range-separator="至"
                                        start-placeholder="开始日期"
                                        end-placeholder="结束日期">
                ...
                <el-tabs v-model="leftTopActiveName" @tab-click="leftHandleClick" style="height: 400px;">
                    <el-tab-pane label="未处理" name="0">
                        <el-table :data="orderListData" fit
                            ...
                            <el-table-column v-for="item in orderListHead" :label="item.label" align="center"
                                             :width="item.width" :key="item.label">
                                <template slot-scope="scope">
                                    <span v-if="item.type=='checkbox'">
                                        <span v-if="item.inProp">
                                             <el-checkbox v-model="scope.row[item.prop][item.inProp]"
            ...
            <el-tab-pane label="订单明细" name="1">
                ...
                <el-form :model="orderDtlFormVal" class="quarter_turn">
                    <el-form-item v-for="item in orderDtlFormHead" :key="item.label" :label="item.label"
                                  :prop="item.prop">
                        <span v-if="item.type=='text'">
                         <span v-if="item.inProp">
                            <el-input v-model.trim="orderDtlFormVal[item.prop][item.inProp]" :placeholder="item.holder"
                                      disabled></el-input>
                        </span>
                        <span v-else>
                          <el-input v-model.trim="orderDtlFormVal[item.prop]" :placeholder="item.holder"
                                    disabled></el-input>
                    </span>
                    </span>
                        <span v-else-if="item.type=='number'">
                       <el-input type="number" v-model.trim="orderDtlFormVal[item.prop]" :placeholder="item.holder"
                                 disabled></el-input>
                    </span>
                        <span v-else-if="item.type=='textarea'">
                         <el-input type="textarea" v-model.trim="orderDtlFormVal[item.prop]"
                                   :placehode="item.holder"></el-input>
                    </span>
                    </el-form-item>
                </el-form>
                <!-- 下方信息 -->
                <el-tabs v-model="rightActiveName" @tab-click="rightHandleClick">
                    <el-tab-pane label="商品明细" name="0">
                        <el-table :data="proDtlData" fit>
                            <el-table-column v-for="item in orderDtlHead[rightActiveName]" :label="item.label"
                                             align="center" :width="item.width" :key="item.label">
                                <template slot-scope="scope">
                                    <span v-if="item.type=='checkbox'">
                                        <el-checkbox v-model="scope.row[item.prop]" disabled></el-checkbox>
                                    </span>
                            ...
                            <el-table-column type="expand" fixed="left">
                                <template slot-scope="scope">
                                    <el-table :data="scope.row['productComp']" fit>
                                        <el-table-column v-for="item in proCompHead" :label="item.label" align="center"
                                                         :width="item.width" :key="item.label">
                                            <template slot-scope="scope">
                                <span v-if="item.prop">
                                    <span v-if="item.type=='checkbox'">
                    ...
                    <el-tab-pane label="优惠列表" name="6">

                    </el-tab-pane>
                    <el-tab-pane label="订单图片" name="7">

                    </el-tab-pane>
                    <el-tab-pane label="电子面单" name="8">
        ...
        <el-dialog title="商品明细" :visible.sync="proMask" :class="{'more-forms':moreForms,'threeParts':threeParts}">
            ...
            <div class="searchBox">
                <span>
                    <label>商品编码</label>
                    <el-input v-model.trim="proQuery.commodity_code" clearable placeholder="请输入商品编码"
                              @keyup.enter.native="proQueryClick"></el-input>
                </span>
                <span>
                    <label>子件编码</label>
                    <el-input v-model.trim="proQuery.component_code" clearable placeholder="请输入子件编码"
                              @keyup.enter.native="proQueryClick"></el-input>
                </span>
                <span>
                     <label>商品类别</label>
                   <el-select v-model="proQuery.shops_id" clearable placeholder="请选择商品类别"
                ...
                <span>
                    <label>商品简称</label>
                    <el-input v-model.trim="proQuery.short_name" clearable placeholder="请输入子件编码"
                              @keyup.enter.native="proQueryClick"></el-input>
                </span>
                <span>
                     <label>组合筛选</label>
            ...
            <el-table :data="proVal" fit height="250" :row-class-name="proCName" @row-click="proRowClick">
                <el-table-column v-for="item in proHead" :label="item.label" align="center" :width="item.width"
                                 :key="item.label">
                    <template slot-scope="scope">
                       <span v-if="item.prop">
            ...
            <el-table :data="proSkuVal" fit height="230" :row-class-name="proSkuCName" @row-click="proSkuRowClick">
                <el-table-column v-for="item in proSkuHead" :label="item.label" align="center" :width="item.width"
                                 :key="item.label">
                    <template slot-scope="scope">
                        <span v-if="item.prop=='newData'">
                            <span v-if="proCompRowIndex == 'index'+scope.$index">
                                <span v-if="item.type=='number'">
                                    <span v-if="item.inProp=='quantity'">
                                         <el-input size="small" type="number"
                                                   ...
                                                   :placeholder="item.holder" @input="quantityChg"></el-input>
                                    </span>
                                    <span v-else>
                                         <el-input size="small" type="number"
                                                   v-model.trim="scope.row[item.prop][item.inProp]"
                                                   :placeholder="item.holder"></el-input>
                                    </span>
                                </span>
                                ...
                                <span v-else>
                                   <el-input size="small" v-model.trim="scope.row[item.prop][item.inProp]"
                                             :placeholder="item.holder"></el-input>
                                </span>
                            </span>
                            <span v-else>
    ...
    <img slot="reference" :src="scope.row[item.prop]" :alt="scope.row[item.alt]">
        ...
        <!--修改  样式不再未bigDialog-->
        <el-dialog title="修改明细" :visible.sync="updateStorageMask"
                   :class="{'more-forms':moreForms,'threeParts':threeParts}" class="bigDialog">
            <div class="clearfix">
                <span style="float: left">物流信息</span>
            </div>
            <!--取消hidepart-->
            <el-form :model="updateStorageFormVal" :rules="addStorageFormRules" class="storageUpdateForm"
                     id="form">
                <el-form-item v-for="(item,index) in addStorageFormHead" :key="index" :label="item.label"
                              :prop="item.prop">
                    <span v-if="item.type=='text'">
                         <span v-if="item.inProp">
                            <el-input v-model.trim="updateStorageFormVal[item.prop][item.inProp]"
                                      :placeholder="item.holder" :disabled="item.addChgAble"></el-input>
                        </span>
                        <span v-else>
                          <el-input v-model.trim="updateStorageFormVal[item.prop]" :placeholder="item.holder"
                                    :disabled="item.addChgAble"></el-input>
                    </span>
                    </span>
                    <span v-else-if="item.type=='number'">
                        <span v-if="item.prop=='deliver_goods_fee' || item.prop=='move_upstairs_fee' || item.prop=='installation_fee'">
                           <el-input type="number" v-model.trim="updateStorageFormVal[item.prop]"
                                     :placeholder="item.holder" :disabled="item.addChgAble" @input="formChg"></el-input>
                        </span>
                        <span v-else>
                            <el-input type="number" v-model.trim="updateStorageFormVal[item.prop]"
                                      :placeholder="item.holder" :disabled="item.addChgAble"></el-input>
                    ...
                    <span v-else-if="item.type=='select'">
                             <el-select v-model="updateStorageFormVal[item.prop]" :placeholder="item.holder"
                    ...
                    <span v-else-if="item.type=='textarea'">
                         <el-input type="textarea" v-model.trim="updateStorageFormVal[item.prop]"
                                   :placehode="item.holder"></el-input>
                    </span>
                    <span v-else-if="item.type=='checkbox'">
                            <el-checkbox v-model="updateStorageFormVal[item.prop]"
                                         :disabled="item.chgAble"></el-checkbox>
                        </span>
                    <span v-else-if="item.type=='radio'">
                            <el-radio v-model="updateStorageFormVal[item.prop]"
                                      label="volume">{{item.choiceName[0]}}</el-radio>
                            <el-radio v-model="updateStorageFormVal[item.prop]"
                                      label="weight">{{item.choiceName[1]}}</el-radio>
                        </span>
                    <span v-else-if="item.type=='DatePicker'">
                            <el-date-picker v-model="updateStorageFormVal[item.prop]" type="date" format="yyyy-MM-dd"
            ...
            <!--<el-table-column v-for="item in addHead[updateActiveName]" :label="item.label" align="center"-->
            <!--:width="item.width" :key="item.label">-->
            <!--<template slot-scope="scope">-->
            <!--<span v-if="item.prop=='newData'">-->
            ...
            <!--<el-table-column label="操作" width="90" align="center" fixed="right">-->
            <!--<template slot-scope="scope">-->
            <!--<el-button size="mini" type="danger" @click="updateDelPro(scope.row,scope.$index)">删除-->
            <!--</el-button>-->
            <!--</template>-->
            <!--</el-table-column>-->
            <!--</el-table>-->
            ...
            <!--<el-form-item v-for="(item,index) in addHead[updateActiveName]" :key="index" :label="item.label"-->
            <!--:prop="item.label">-->
            <!--<span v-if="item.type=='number'">-->
            <!--<el-input type="number" v-model.trim="updateReceiveInfo[item.prop]"-->
            <!--:placeholder="item.holder"></el-input>-->
            <!--</span>-->
            ...
            <!--</span>-->
            <!--<span v-else>-->
            <!--<el-input v-model.trim="updateReceiveInfo[item.prop]"-->
            <!--:placeholder="item.holder"></el-input>-->
            <!--</span>-->
            <!--</el-form-item>-->
            <!--</el-form>-->
            ...
            <!--:row-class-name="addExpenseRCName">-->
            <!--<el-table-column v-for="item in addHead[updateActiveName]" :label="item.label" align="center"-->
            <!--:width="item.width" :key="item.label">-->
            ...
            <!--<el-table-column label="操作" width="90" align="center" fixed="right">-->
            <!--<template slot-scope="scope">-->
            <!--<el-button size="mini" type="danger" @click="updateDelExpense(scope.row,scope.$index)">-->
            <!--删除-->
            <!--</el-button>-->
            <!--</template>-->
            <!--</el-table-column>-->
            ...
            <div slot="footer" class="dialog-footer clearfix">
                <div style="float: right">
                    <el-button type="primary" @click="updateStorageConfirm">确定</el-button>
                    <el-button @click="updateStorageCancel">取消</el-button>
                </div>
            </div>
        </el-dialog>
        ...
        <el-dialog title="拆分订单" :visible.sync="splitMask" :class="{'more-forms':moreForms,'threeParts':threeParts}">
            ...
            <el-table :data="splitVal" fit height="300" @row-click="splitRowClick" :row-class-name="splitCName">
                <el-table-column v-for="item in splitHead" :label="item.label" align="center" :width="item.width"
                                 :key="item.label">
                    <template slot-scope="scope">
                        <span v-if="item.prop=='newData'">
                            <span v-if="splitRowIndex == 'index'+scope.$index">
                                <el-input size="small" v-model.trim="scope.row[item.prop][item.inProp]"
                                          @input="numChg"></el-input>
                            </span>
                            <span v-else>
                                {{scope.row[item.prop][item.inProp]}}
                            </span>
...
<script>
    ...
    */
        ...
        methods: {
            ...
            ,
            addStorageConfirm() {
                let forData = this.addStorageFormVal;
                let submitData = {
            ...
            * 折叠  没有partHide*/
            ...
            ,
            updateStorageConfirm() {
                let forData = this.updateStorageFormVal;
                let submitData = {
                    shops_id: forData.shops_id,
                    member_nick: forData.member_nick,
                    logistics_id: forData.logistics_id,
