      ...
      <el-tab-pane label="订单列表" name="0">
        <div>
          ...
          <div class="searchBox">
            ...
            <span>
              <label>支付时间</label>
              <el-date-picker v-model="searchBox.payment_date" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
              </el-date-picker>
            </span>
            <span>
              <label>订单状态</label>
        ...
        <el-tabs v-model="rightActiveName" @tab-click="rightHandleClick">
          ...
          <el-tab-pane label="订单信息" name="1">
            <el-table :data="orderInfoData" fit>
              <el-table-column v-for="item in orderInfoHead" :label="item.label" align="center" :width="item.width" :key="item.label">
                <template slot-scope="scope">
                  <span>
                    {{scope.row[item.prop]}}
                  </span>
          ...
          <el-tab-pane label="门店收款明细" name="12">
            <el-table :data="receiveDtlData" fit>
              <el-table-column v-for="item in receiveDtlHead" :label="item.label" align="center" :width="item.width" :key="item.label">
                <template slot-scope="scope">
                  <span v-if="item.inProp">
                    <span v-if="item.inData">
                      {{scope.row[item.prop]['data']?scope.row[item.prop]['data'][0][item.inProp]:''}}
          ...
          <el-tab-pane label="库存占用" name="15">
            <el-table :data="warehouseData" fit>
              <el-table-column v-for="item in warehouseHead" :label="item.label" align="center" :width="item.width" :key="item.label">
                <template slot-scope="scope">
                  <span>
                    {{item.inProp?scope.row[item.prop][item.inProp]:scope.row[item.prop]}}
                  </span>
