<template>
  <div>
    <el-tabs v-model="activeName" @tab-click="outerHandleClick">
      <div>
        ...
        <div class="searchBox">
          ...
          <span>
            <label>还款时间</label>
            <el-date-picker v-model="searchBox.refund_time" type="daterange" range-separator="至" start-placeholder="开始日期"
      ...
      <el-tabs v-model="orderListActiveName" @tab-click="leftHandleClick" style="height: 400px;">
        <el-tab-pane label="已审核" name="0">
          <el-table :data="untreatedOrderListData" fit @selection-change="handleSelectionChange" v-loading="loading"
            height="350" @row-click="orderListRowClick" @row-dblclick="orderDbClick">
        ...
        <el-tab-pane label="已客审" name="1">
          <el-table :data="treatedOrderListData" fit @selection-change="handleSelectionChange" v-loading="loading"
            height="350" @row-click="orderListRowClick" @row-dblclick="orderDbClick">
            <el-table-column type="selection" width="95" align="center" :checked="checkboxInit">
            </el-table-column>
            <el-table-column v-for="item in orderListHead" :label="item.label" align="center" :width="item.width" :key="item.label">
      ...
      <el-tabs>
        <el-tab-pane label="详细信息" name="0">
          <el-form :model="detailRefundOrderData">
            <el-form-item v-for="(item,index) in detailRefundOrderHead" :label="item.label" align="center" :prop="item.prop"
              :key="index">
              <span v-if="item.type=='text'">
                <span v-if="item.inProp">
                  <el-input v-model.trim="detailRefundOrderData[item.prop][item.inProp]" :placeholder="item.holder"
              ...
              <span v-else-if="item.type=='number'">
                <span v-if="item.prop=='deliver_goods_fee' || item.prop=='move_upstairs_fee' || item.prop=='installation_fee'">
                  <el-input type="number" v-model.trim="detailRefundOrderData[item.prop]" :placeholder="item.holder"
                    :disabled="item.addChgAble" @input="formChg"></el-input>
                </span>
                <span v-else>
                  <el-input type="number" v-model.trim="detailRefundOrderData[item.prop]" :placeholder="item.holder"
                    :disabled="item.addChgAble"></el-input>
              ...
              <span v-else-if="item.type=='textarea'">
                <el-input type="textarea" v-model.trim="detailRefundOrderData[item.prop]" :placehode="item.holder"
                  :disabled="item.addChgAble"></el-input>
              </span>
              ...
              <span v-else-if="item.type=='DatePicker'">
                <el-date-picker v-model="detailRefundOrderData[item.prop]" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd"
        ...
        <el-tab-pane label="多张图片" name="2">
          <el-table :data="refundReasonTabData">
            <el-table-column v-for="item in refundReasonTabHead" :label="item.label" align="center" :width="item.width"
              :key="item.prop">
              <template slot-scope="scope">
                <span v-if="item.type=='checkbox'">
                  <el-checkbox v-model="scope.row[item.prop]" disabled></el-checkbox>
    ...
    <!--修改-->
    <el-dialog title="修改退款单" :visible.sync="updateRefundOrderMask" :class="{'more-forms':moreForms,'threeParts':threeParts}"
      class="bigDialog">
      <div class="clearfix">
        ...
        <label>{{addRefundOrderFormVal.order_sn}}</label>
        <br>
        <label>addRefundReasonCurIndex:</label>
        <label>{{addRefundReasonCurIndex}}</label>
        <br>
        <label>updateRefundReasonCurIndex:</label>
        <label>{{updateRefundReasonCurIndex}}</label>
        <br>
        <label>addRefundReasonCurIndexNum:</label>
        <label>{{addRefundReasonCurIndexNum}}</label>
        <br>
        <label>updateRefundReasonCurIndexNum:</label>
        <label>{{updateRefundReasonCurIndexNum}}</label>
        <br>
        <label>addRefundReasonUploadIndex:</label>
        <label>{{addRefundReasonUploadIndex}}</label>
        <br>
        <label>updateRefundReasonUploadIndex:</label>
        <label>{{updateRefundReasonUploadIndex}}</label>
        <br>
      </div>
      <el-form :model="updateRefundOrderFormVal" :rules="addRefundFormRules" class="customerUpdateForm" id="form">
        <el-form-item v-for="(item,index) in addRefundFormHead" :key="index" :label="item.label" :prop="item.prop">
          <span v-if="item.type=='text'">
            <span v-if="item.inProp">
              <el-input v-model.trim="updateRefundOrderFormVal[item.prop][item.inProp]" :placeholder="item.holder"
                :disabled="item.addChgAble"></el-input>
            </span>
            <span v-else>
          ...
          <span v-else-if="item.type=='number'">
            <span v-if="item.prop=='deliver_goods_fee' || item.prop=='move_upstairs_fee' || item.prop=='installation_fee'">
              <el-input type="number" v-model.trim="updateRefundOrderFormVal[item.prop]" :placeholder="item.holder"
                :disabled="item.addChgAble" @input="formChg"></el-input>
            </span>
            <span v-else>
              <el-input type="number" v-model.trim="updateRefundOrderFormVal[item.prop]" :placeholder="item.holder"
          ...
          <span v-else-if="item.type=='DatePicker'">
            <el-date-picker v-model="updateRefundOrderFormVal[item.prop]" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd"
              placeholder="选择日期">
            </el-date-picker>
          </span>
        </el-form-item>
      ...
      <el-table :data="updateRefundOrderFormVal.refund_reason" fit @row-click="refundReasonRowClick" :row-class-name="addProRCName">
        <el-table-column v-for="(item,index) in refundReasonHead" :label="item.label" align="center" :width="item.width"
          :key="index">
          <template slot-scope="scope">
  ...
  data() {
    ...
    return {
      ...
      addRefundFormRules: {
        ...
        shops_id: [
          { required: true, message: '所属店铺必选', trigger: 'blur' }
        ],
        refund_reason_type_id: [
...
*/
  ...
  methods: {
    ...
    fetchData() {
      ...
      switch (index) {
        ...
        this.newOpt[6].nClick = false;
          this.$fetch(
            this.urls.customerservicerefunds + '/searchastreated',{
              shops_id:this.searchBox.shops_id,
              order_sn:this.searchBox.order_sn,
              buyer_nick:this.searchBox.buyer_nick,
              buyer_name:this.searchBox.buyer_name,
    ...
    updateTabAddMoreRefundReason() {
      ...
      this.updateRefundReasonUploadIndex = `upload${
    ...
    handleSelectionChange(val) {
      ...
      if (index == 0) {
        if (this.mergerIds.length == 1) {
          if (val[0].refund_order_status == "售后锁定") {
    ...
    lockOrder() {
      ...
      this.$put(
        this.urls.customerservicerefunds + '/' + id + '/aslockorunlock'
      ).then(
        () => {
          this.refresh();
          this.$message({
    ...
    handleAudit() {
      let id = this.checkboxId ? this.checkboxId : this.orderListTabCurRowId;
      this.$put(this.urls.customerservicerefunds + '/' + id + '/asaudit').then(
        () => {
          this.refresh();
          this.$message({
            message: '审核成功',
    ...
    handleUnAudit() {
      let id = this.checkboxId ? this.checkboxId : this.orderListTabCurRowId;
      this.$put(
        this.urls.customerservicerefunds + '/' + id + '/asunaudit'
      ).then(
        () => {
          this.refresh();
          this.$message({
    ...
    refuse() {
      ...
      this.$fetch(this.urls.customerservicerefunds + '/' + id, {
        ...
        res => {
          this.updateRefundOrderFormVal = res;
          this.updateRefundOrderFormVal.refund_order_status = 10;
        },
        err => {
          if (err.response) {
            let arr = err.response.data.errors;
    ...
    refuseConfirm() {
      ...
      this.$patch(
        ...
        () => {
          this.$put(
            this.urls.customerservicerefunds + '/' + id + '/asrefuse'
          ).then(
            () => {},
            err => {
              if (err.response) {
