      ...
      <el-tabs>
        ...
        <el-tab-pane label="退款责任方" name="2">
          <el-table :data="responsiblePartyData" fit>
            <el-table-column v-for="item in refundResponsiblePartyHead" :label="item.label" align="center" :width="item.width"
              :key="item.prop">
              <template slot-scope="scope">
                <span v-if="item.type=='checkbox'">
                  <el-checkbox v-model="scope.row[item.prop]" disabled></el-checkbox>
    ...
    handleSelectionChange(val) {
      ...
      if (index == 0) {
        if (this.mergerIds.length == 1) {
          if (val[0].refund_order_status == "财务锁定") {
