  ...
  <div>
    ...
    <el-tabs v-model="topActiveName" @tab-click="clickTopTabs">
      <el-tab-pane label="新建" name="0">
        <el-table :data="newData" fit @selection-change="handleSelectionChange" v-loading="loading" height="300" @row-click="stockRowClick">
      ...
      <el-tab-pane label="已提交" name="1">
        <el-table :data="partData" fit @selection-change="handleSelectionChange" v-loading="loading" height="300" @row-click="stockRowClick">
          <el-table-column type="selection" width="95" align="center" :checked="checkboxInit"></el-table-column>
          <el-table-column v-for="item in stockHead" :label="item.label" align="center" :width="item.width" :key="item.prop">
            <template slot-scope="scope">
              <span v-if="item.type=='checkbox'">
      ...
      <el-tab-pane label="已入库" name="2">
        <el-table :data="finishData" fit @selection-change="handleSelectionChange" v-loading="loading" height="300" @row-click="stockRowClick">
          <el-table-column type="selection" width="95" align="center" :checked="checkboxInit"></el-table-column>
          <el-table-column v-for="item in stockHead" :label="item.label" align="center" :width="item.width" :key="item.prop">
            <template slot-scope="scope">
              <span v-if="item.type=='checkbox'">
    ...
    <el-tabs v-model="btmActiveName" @tab-click="clickBtmTabs">
      ...
      <el-tab-pane label="入库图片" name="1">入库图片</el-tab-pane>
    </el-tabs>

    <!--新建-->
    <el-dialog title="新建其他入库单" :visible.sync="addStockMask" :class="{'more-forms':moreForms,'threeParts':threeParts}">
...
export default {
  ...
  methods: {
              ...
              if (res.data[0]["stockInDetails"]["data"].length > 0) {
                res.data[0]["stockInDetails"]["data"].map(item => {
                  ...
                  let list = {
                    ...
                    purchase_order_no:
                      purchaseList["purchase"].purchase_order_no,
    ...
    addStockConfirm() {
      ...
      this.$post(this.urls.stockins, submitData).then(
        () => {
    ...
    stockChg(value) {
      if (this.addStockMask) {
        if (value > this.addStockRData.maxStockIn - 0) {
          ...
          this.addStockRData.total_fee =
            (this.addStockRData.purchase_cost - 0) * value;
        }
      } else {
        if (value > this.updateStockRData.maxStockIn - 0) {
          ...
          this.updateStockRData.total_fee =
            (this.updateStockRData.purchase_cost - 0) * value;
        }
      }
    },
    /*商品明细*/
    proQueryClick() {
      this.proDtlVal = [];
      this.$fetch(this.urls.purchaselists, {
        status: true,
        include:
          "purchase.user,purchaseDetails.productComponent.product,purchaseDetails.purchaseList"
      }).then(
        res => {
          res.data.map(item => {
            if (item["purchaseDetails"]["data"].length > 0) {
              item["purchaseDetails"]["data"].map(purDtlItem => {
                let list = {
                  ...
                  commodity_code:
                    purDtlItem["productComponent"]["product"].commodity_code,
                  short_name:
                    purDtlItem["productComponent"]["product"].short_name,
                  component_code: purDtlItem["productComponent"].component_code,
                  spec: purDtlItem["productComponent"].spec,
                  package_quantity:
                    purDtlItem["productComponent"].package_quantity,
                  ...
                  total_fee: "",
                  maxStockIn:
                    purDtlItem.purchase_quantity -
                    0 -
                    (purDtlItem.stock_in_count - 0),
    ...
    updateStock() {
      if (this.newOpt[1].nClick) {
        ...
        let id = this.checkboxId ? this.checkboxId : this.stockRowId;
        this.$fetch(this.urls.stockins + "/" + id, {
          include:
            "stockInDetails.productComponent.product,stockInDetails.purchaseDetail.purchaseList.purchase.user"
        }).then(
          res => {
            ...
            if (res["stockInDetails"]["data"].length > 0) {
              res["stockInDetails"]["data"].map(item => {
                ...
                let list = {
                  id: item.id,
                  purchase_order_no:
                    purDtl["purchaseList"]["purchase"].purchase_order_no,
                  ...
                  total_fee: item.total_fee,
                  maxStockIn:
                    purDtl.purchase_quantity - 0 - (purDtl.stock_in_count - 0),
                  created_at:
                    purDtl["purchaseList"]["purchase"]["user"].created_at,
                  purchase_details_id: item.purchase_details_id,
                  product_components_id: item.product_components_id
                };
    ...
    updateStockRCName({ row, rowIndex }) {
      row.index = rowIndex;
    },
    ...
    updateStockConfirm() {
      ...
      let id = this.checkboxId ? this.checkboxId : this.stockRowId;
      this.$patch(this.urls.stockins + "/" + this.id, submit).then(
        () => {
    ...
    updateStockDel(row, index) {
      this.$del(this.urls.stockindetails + "/" + row.id).then(
        () => {
    ...
    delSingle(row, e) {
      ...
      this.delId = row.id;
      this.delUrl = row["stockInDetails"]
        ? this.urls.stockins
        : this.urls.stockindetails;
    },
    cancelD() {
      this.showDel = false;
      this.$message({
    ...
    delBatch() {
      if (this.ids.length === 0) {
        ...
        this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$del(this.urls.stockins, { ids: this.ids }).then(
              () => {
                this.$message({
                  message: "删除成功",
                  type: "success"
    ...
    handlePagChg(page) {
      ...
      switch (index) {
        case 0:
          ...
          this.newOpt[4].nClick = true;
          this.newOpt[5].nClick = true;
          this.$fetch(this.urls.stockins + "?page=" + page, {
            is_submit: false,
            include:
    ...
    doCommit() {
      if (this.newOpt[3].nClick) {
        ...
        if (this.multipleSelection.length == 0) {
          ...
          let id = this.checkboxId ? this.checkboxId : this.stockRowId;
          this.$put(this.urls.stockins + "/" + id + "/submit").then(
            () => {
              this.$message({
                message: "提交成功!",
                type: "success"
    ...
    doPrinter() {
      if (!this.newOpt[7].nClick) {
        let id = this.checkboxId ? this.checkboxId : this.stockRowId;
        this.$put(this.urls.stockins + "/" + id + "/print").then(
          () => {
            this.printBtn();
            this.$message({
              message: "打印成功!",
    ...
    doAudit() {
      if (!this.newOpt[4].nClick) {
        let id = this.checkboxId ? this.checkboxId : this.stockRowId;
        this.$put(this.urls.stockins + "/" + id + "/audit").then(
          () => {
            this.$message({
              message: "审核成功!",
              type: "success"
    ...
    unAudit() {
      if (!this.newOpt[5].nClick) {
        let id = this.checkboxId ? this.checkboxId : this.stockRowId;
        this.$put(this.urls.stockins + "/" + id + "/unaudit").then(
          () => {
            this.$message({
              message: "退审成功!",
              type: "success"
