<template>
  <div>
    ...
    <el-tabs v-model="btmActiveName" @tab-click="clickBtmTabs">
      ...
      <el-tab-pane label="图片信息" name="1">入库图片</el-tab-pane>
    </el-tabs>
...
export default {
  ...
  methods: {
    ...
    addPurReturnRCName({ row, rowIndex }) {
      row.index = rowIndex;
    },
    ...
    addPurReturnConfirm() {
      ...
      this.$post(this.urls.purchasereturns, submitData).then(
        () => {
      ...
      if (this.addPurReturnMask) {
        this.proDtlVal.map(item => {
          if (item.newData.purchase_return_quantity > 0) {
            if (
              item.newData["suppliers_id"] !== undefined &&
              item["newData"].price_differences !== undefined &&
              item["newData"].purchase_return_types_id !== undefined
            ) {
    ...
    updatePurReturn() {
      ...
      if (this.newOpt[1].nClick) {
        ...
        this.$fetch(this.urls.purchasereturns + "/" + id, {
          include:
            "purchaseReturnDetails.stock.productComponent,purchaseReturnDetails.stock.product,purchaseReturnDetails.stock.warehouse,purchaseReturnDetails.supplier,purchaseReturnDetails.purchaseReturnType"
        }).then(
          res => {
    ...
    updatePurReturnRCName({ row, rowIndex }) {
      row.index = rowIndex;
    },
    ...
    updatePurReturnDel(row, index) {
      if (row.stocks_id) {
        this.$del(this.urls.purchasereturndetails + "/" + row.id).then(
          () => {
    ...
    updatePurReturnConfirm() {
      ...
      let id = this.checkboxId ? this.checkboxId : this.curRowId;
      this.$patch(this.urls.purchasereturns + "/" + id, updateData).then(
        () => {
    ...
    delSingle(row, e) {
      ...
      this.delId = row.id;
      this.delUrl = row["purchaseReturnDetails"]
        ? this.urls.purchasereturns
        : this.urls.purchasereturndetails;
    },
    cancelD() {
      this.showDel = false;
      this.$message({
    ...
    delBatch() {
      if (this.ids.length === 0) {
        ...
        this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$del(this.urls.purchasereturns, { ids: this.ids }).then(
              () => {
                this.$message({
                  message: "删除成功",
                  type: "success"
    ...
    bfSubmit() {
      if (this.newOpt[3].nClick) {
        ...
        let id = this.checkboxId ? this.checkboxId : this.curRowId;
        this.$put(this.urls.purchasereturns + "/" + id + "/submit").then(
          () => {
            this.refresh();
            this.$message({
              message: "提交成功",
    ...
    bfReject() {
      if (this.newOpt[4].nClick) {
        ...
        let id = this.checkboxId ? this.checkboxId : this.curRowId;
        this.$put(this.urls.purchasereturns + "/" + id + "/reject").then(
          () => {
            this.refresh();
            this.$message({
              message: "驳回成功",
    ...
    bfAudit() {
      if (this.newOpt[5].nClick) {
        ...
        let id = this.checkboxId ? this.checkboxId : this.curRowId;
        this.$put(this.urls.purchasereturns + "/" + id + "/audit").then(
          () => {
            this.refresh();
            this.$message({
              message: "审核成功",
    ...
    bfUnAudit() {
      if (this.newOpt[6].nClick) {
        ...
        let id = this.checkboxId ? this.checkboxId : this.curRowId;
        this.$put(this.urls.purchasereturns + "/" + id + "/unaudit").then(
          () => {
            this.refresh();
            this.$message({
              message: "退审成功",
      ...
      switch (index) {
        case 0:
          ...
          this.$fetch(this.urls.purchasereturns + "?page=" + page, {
            is_submit: false,
            include:
              "purchaseReturnDetails.stock.productComponent,purchaseReturnDetails.stock.product,purchaseReturnDetails.supplier,purchaseReturnDetails.purchaseReturnType"
          }).then(
            res => {
              this.noSubmitData = res.data;
              this.loading = false;
