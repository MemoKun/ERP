  ...
  methods: {
              ...
              if (res.data[0]["cancelPurchaseDetails"]["data"].length >= 0) {
                res.data[0]["cancelPurchaseDetails"]["data"].map(item => {
                  let list = {
                    ...
                    commodity_code:
                      item["purchaseDetail"]["productComponent"]["product"]
                        .commodity_code,
                    short_name:
                      item["purchaseDetail"]["productComponent"]["product"]
                        .short_name,
                    component_code:
                      item["purchaseDetail"]["productComponent"].component_code,
          ...
          let list = {
            ...
            commodity_code:
              item["purchaseDetail"]["productComponent"]["product"]
                .commodity_code,
            short_name:
              item["purchaseDetail"]["productComponent"]["product"].short_name,
            component_code:
              item["purchaseDetail"]["productComponent"].component_code,
            spec: item["purchaseDetail"]["productComponent"].spec,
            suppliers_id: item["purchaseDetail"].suppliers_id,
    ...
    selectPurOrder() {
      if (this.cancelPurMask) {
        ...
        this.$fetch(this.urls.purchases, {
          ...
          status: true,
          include:
            "user,purchaseLists.purchaseDetails.productComponent,purchaseLists.purchase,purchaseLists.combination.product"
        }).then(
          res => {
            res.data.map(item => {
              obj.push(item);
    ...
    autoFill() {
      ...
      if (this.cancelPurMask) {
        this.addStockVal.map(item => {
          item.cancel_purchase_quantity =
            item.purchase_quantity - 0 - (item.stock_in_count - 0);
        });
      } else {
        this.updateStockVal.map(item => {
          item.cancel_purchase_quantity =
    ...
    confirmAdd() {
      if (this.cancelPurMask) {
        ...
        this.$post(this.urls.cancelpurchases, submitForm).then(
          () => {
            this.cancelPurMask = false;
            this.refresh();
            this.$message({
        ...
        this.$patch(this.urls.cancelpurchases + "/" + id, submitForm).then(
          () => {
            this.updateCancelPurMask = false;
            this.refresh();
            this.$message({
    ...
    updatePur() {
      if (this.newOpt[1].nClick) {
        ...
        this.$fetch(this.urls.cancelpurchases + "/" + id, {
          include:
            "purchase,cancelPurchaseDetails.cancelPurchase,cancelPurchaseDetails.purchaseDetail,cancelPurchaseDetails.purchaseDetail.productComponent,cancelPurchaseDetails.purchaseDetail.productComponent.product"
        }).then(
          res => {
            ...
            let resData = {
              id: res.id,
              img_url:
                res["cancelPurchaseDetails"]["data"][0]["purchaseDetail"][
                  "productComponent"
                ].img_url,
              ...
              stock_in_count:
                res["cancelPurchaseDetails"]["data"][0]["purchaseDetail"]
                  .stock_in_count,
              cancel_purchase_quantity:
                res["cancelPurchaseDetails"]["data"][0]
                  .cancel_purchase_quantity,
              purchase_details_id:
                res["cancelPurchaseDetails"]["data"][0].purchase_details_id
            };
    ...
    delSingle(row, e) {
      ...
      this.delId = row.id;
      this.delUrl = row["cancelPurchaseDetails"]
        ? this.urls.cancelpurchases
        : this.urls.cancelpurchasedetails;
    },
    cancelD() {
      this.showDel = false;
      this.$message({
    ...
    delBatch() {
      if (this.ids.length === 0) {
        ...
        this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$del(this.urls.cancelpurchases, { ids: this.ids }).then(
              () => {
                this.$message({
                  message: "删除成功",
                  type: "success"
    ...
    /*分页*/
    handlePagChg(page) {
      this.$fetch(this.urls["cancelpurchases"] + "?page=" + page).then(res => {
        this.topActiveName == "0"
          ? (this.newData = res.data)
          : (this.partData = res.data);
      });
    },
    ...
    submit() {
      let id = this.checkboxId ? this.checkboxId : this.curRowId;
      this.$put(this.urls.cancelpurchases + "/" + id + "/submit").then(
        () => {
          this.refresh();
          this.$message({
            message: "提交成功",
