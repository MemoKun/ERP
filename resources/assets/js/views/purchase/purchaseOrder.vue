    ...
    <div class="clearfix">
      ...
      <div style="overflow: hidden">
        <el-tabs v-model="rightActiveName" @tab-click="clickRightTabs">
          ...
          <el-tab-pane label="唯一码" name="1">唯一码</el-tab-pane>
        </el-tabs>
      </div>
    </div>
  ...
  methods: {
    ...
    purRowClick(row) {
      ...
      if (row["purchaseLists"]["data"][0]) {
        this.purDetailsVal =
          row["purchaseLists"]["data"][0]["purchaseDetails"]["data"];
      } else {
        this.purDetailsVal = [];
      }
    ...
    confirmAddPur() {
      let submitData = this.addPurchaseForm;
      ...
      this.$post(this.urls.purchases, submitData).then(
        () => {
          this.$message({
            message: "新建采购单成功",
            type: "success"
    ...
    proRowClick(row) {
      ...
      this.proCompVal =
        row.combinations["data"][0]["productComponents"]["data"];
    },
    ...
    updatePurSkuDel(row, index) {
      ...
      if (row.is_newAdd) {
        ...
        this.$del(this.urls.purchaselists + "/" + row.id).then(
          () => {
            this.updatePurSkuVal.splice(index, 1);
            this.$message({
              message: "删除采购sku成功",
      ...
      if (this.updateCompFlag) {
        if (formData.compData) {
          ...
          if (formData["purchaseDetails"]["data"][row.index].productComponent) {
            /*修改*/
            formData["purchaseDetails"]["data"][row.index][
              "productComponent"
            ] = row;
            this.updateCompFlag = false;
          } else {
    ...
    updateDelPurComp(row, index) {
      ...
      if (this.updateSkuRow.is_newAdd) {
        ...
        if (row.is_newAdd && row.combId) {
          /*替换*/
          this.$del(this.urls.purchasedetails + "/" + row.combId).then(
            () => {
              this.updatePurCompVal.splice(index, 1);
              this.updatePurForm.purchase_lists.map(item => {
          ...
          /*删除*/
          this.$del(this.urls.purchasedetails + "/" + row.compId).then(
            () => {
              this.updatePurCompVal.splice(index, 1);
              this.updatePurForm.purchase_lists.map(item => {
                if (item.id == this.updateSkuRow.id) {
    ...
    confirmUpdatePur(id) {
      if (this.updatePurForm.purchase_lists.length == 0) {
              ...
              if (alreadyList.is_newAdd) {
                ...
                if (alreadyList.combId) {
                  let newComp = {
                    ...
                    purchase_quantity:
                      alreadyList["proPurchaseData"].purchase_quantity,
                    ...
                    warehouse_cost:
                      alreadyList["proPurchaseData"].warehouse_cost,
                    purchase_freight:
                      alreadyList["proPurchaseData"].purchase_freight,
                    commission: alreadyList["proPurchaseData"].commission,
                    discount: alreadyList["proPurchaseData"].discount,
                    wooden_frame_costs:
                      alreadyList["proPurchaseData"].wooden_frame_costs,
                ...
                let alreadyComp = {
                  ...
                  purchase_quantity:
                    alreadyList["productComponent"]["proPurchaseData"]
                      .purchase_quantity,
                  shops_id:
                    alreadyList["productComponent"]["proPurchaseData"].shops_id,
                  suppliers_id:
                    alreadyList["productComponent"]["proPurchaseData"]
                      .suppliers_id,
                  purchase_cost:
                  ...
                  commission:
                    alreadyList["productComponent"]["proPurchaseData"]
                      .commission,
                  discount:
                    alreadyList["productComponent"]["proPurchaseData"].discount,
                  wooden_frame_costs:
                    alreadyList["productComponent"]["proPurchaseData"]
                      .wooden_frame_costs,
                  arrival_time:
                    alreadyList["productComponent"]["proPurchaseData"]
                      .arrival_time,
                  remark:
                    alreadyList["productComponent"]["proPurchaseData"].remark
                };
        ...
        this.$patch(this.urls.purchases + "/" + id, addSku).then(
          () => {
            this.$message({
              message: "修改成功",
              type: "success"
      ...
      if (this.newOpt[2].nClick) {
        ...
        if (this.delArr.length === 0) {
          ...
          this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
            ...
            .then(() => {
              this.$del(this.urls.purchases, { ids: this.delArr }).then(
                () => {
                  this.$message({
                    message: "删除成功",
                    type: "success"
    ...
    doCommit() {
      if (this.newOpt[3].nClick) {
        ...
        if (this.multipleSelection.length == 0) {
          ...
          this.$put(this.urls.purchases + "/" + this.updateId + "/submit").then(
            () => {
              this.$message({
                message: "提交成功!",
                type: "success"
    ...
    doPrinter() {
      if (!this.newOpt[10].nClick) {
        this.$put(this.urls.purchases + "/" + this.purRow.id + "/print").then(
          () => {
            this.printBtn();
            this.$message({
              message: "打印成功!",
    ...
    doAudit() {
      if (!this.newOpt[4].nClick) {
        this.$put(this.urls.purchases + "/" + this.purRow.id + "/audit").then(
          () => {
            this.$message({
              message: "审核成功!",
              type: "success"
