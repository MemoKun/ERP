      ...
      <el-tabs>
        <el-tab-pane label="用户信息" name ="0">
            <el-table :data="UserData" fit v-loading="loading" @row-click="logicRowClick" @selection-change="handleSelectionChange">
              ...
              <el-table-column v-for="item in UserHead" :label="item.label" 
              :width="item.width" :key="item.prop" align="center">
      ...
      <el-tabs>
        <el-tab-pane label="物流公司" name ="0">
            <el-table :data="logisticsData" v-loading="loading">
              ...
              <el-table-column v-for="item in logisticsHead" :label="item.label" 
              :width="item.width" :key="item.prop" align="center">
                <template slot-scope="scope">
                  <span>
                    {{item.inProp?scope.row[item.prop][item.inProp]:scope.row[item.prop]}}
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //获取数据
      fetchData(){
        this.$fetch(this.urls.relateLogistics,{include:'relateLogisticsCompany.relateLogistics'})
        .then(res=>{
          this.loading=false;
          this.UserData=res.data;
          this.logisticsData=res.data[0]?res.data[0]['relateLogisticsCompany'].data:[];
        },err=>{
          if (err.response) {
      ...
      addConfirm(){
        ...
        this.$post(this.urls.relateLogistics,data)
          .then(()=>{
            this.addMask = false;
            this.refresh();
            this.$message({
      ...
      addLogisticsCName({row,rowIndex}){row.index = rowIndex},
      ...
      updateUser(){
        ...
        this.$fetch(this.urls.relateLogistics+'/'+id,{include:'relateLogisticsCompany.relateLogistics'})
          .then(res=>{
            this.updateUserForm = {
      ...
      updateConfirm(){
        ...
        this.$patch(this.urls.relateLogistics+'/'+id,data)
          .then(()=>{
      ...
      updateLogisticsDel(row,index){
        if(row.id){
          this.$del(this.urls.relateLogisticsCompany+'/'+row.id)
            .then(()=>{
      ...
      delUser(){
        if (this.ids.length === 0) {
          ...
          this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            this.$del(this.urls.relateLogistics, {ids: this.ids})
              .then(() => {
                this.$message({
                  message: '删除成功',
                  type: 'success'
