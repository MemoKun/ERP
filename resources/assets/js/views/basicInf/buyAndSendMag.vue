    ...
    <div>
      ...
      <el-tab-pane label="买就送赠品" name ="0">
            <el-table :data="giftData" v-loading="loading">
              ...
              <el-table-column v-for="item in giftHead" :label="item.label" 
              :width="item.width" :key="item.label" align="center">
                <template slot-scope="scope">
                  <span v-if="item.type=='checkbox'">
                    <el-checkbox v-model="scope.row[item.prop]" disabled></el-checkbox>
...
import { regionDataPlus, CodeToText, TextToCode } from 'element-china-area-data'
  export default {
    ...
    methods:{
      ...
      //获取数据
      fetchData(){
        this.$fetch(this.urls.buyAndSendPro,{include:'buyAndSendGift.buyAndSendPro'})
          .then(res=>{
            this.loading=false;
      ...
      addGiftName({row,rowIndex}){row.index = rowIndex},
      addGiftRClick(row){
      ...
      addConfirm(){
        ...
        this.$post(this.urls.buyAndSendPro,data)
          .then(()=>{
            this.addMask = false;
            this.refresh();
            this.$message({
      ...
      updatePro(){
        ...
        this.$fetch(this.urls.buyAndSendPro+'/'+id,{include:'buyAndSendGift.buyAndSendPro'})
        .then(res=>{
          this.updateProForm={
            title:res.title,
      ...
      updateGiftDtl(row,index){
        if(row.id){
          this.$del(this.urls.buyAndSendGift+'/'+row.id)
            .then(()=>{
      ...
      updateConfirm(){
        ...
        this.$patch(this.urls.buyAndSendPro+'/'+id,data)
          .then(()=>{
      ...
      delPro(){
        if (this.ids.length === 0) {
          ...
          this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            this.$del(this.urls.buyAndSendPro, {ids: this.ids})
              .then(() => {
                this.$message({
                  message: '删除成功',
                  type: 'success'
      ...
      audit(){
        ...
        this.$put(this.urls.buyAndSendPro+'/'+id+'/audit')
          .then(()=>{
            this.refresh();
            this.$message({
              message:'审核成功',
      ...
      void(){
        let id=this.currentId?this.currentId:this.productsRow.id;
        this.$put(this.urls.buyAndSendPro+'/'+id+'/void')
          .then(()=>{
            this.refresh();
            this.$message({
              message:'作废成功',
