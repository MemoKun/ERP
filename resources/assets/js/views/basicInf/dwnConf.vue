     ...
     <el-tabs>
      <el-tab-pane label="下载配置" name ="0">
            <el-table :data="confData" fit v-loading="confLoading" @selection-change="handleSelectionChange">
              ...
              <el-table-column v-for="item in confHead" :label="item.label" 
...
<script>
  export default {
    ...
    //分页
    handlePagChg(page){
      this.$fetch(this.urls.dwnConf+'?page='+page)
        .then(res=>{
          this.confData=res.data;
        })
    },
    ...
    addConfirm(){
      ...
      this.$post(this.urls.dwnConf,obj)
        .then(()=>{
          this.$message({
            message: '添加成功',
            type: 'success'
    ...
    updateConf(){
      if(this.selections.length==0){
        ...
        this.$fetch(this.urls.dwnConf+'/'+this.currentId)
        .then(res=>{
          this.updateForm={
    ...
    updateConfirm(){
      this.$patch(this.urls.dwnConf+'/'+ this.currentId,this.updateForm)
          .then(()=>{
    ...
    delConf(){
      if(this.delIds.length===0){
        ...
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            this.$del(this.urls.dwnConf,{ids:this.delIds})
              .then(()=>{
                this.$message({
                  message: '删除成功',
                  type: 'success'
