<template>
  <div>
    ...
    <!--商品信息-->
    <el-table :data="productsVal" fit ref="multipleTable" @selection-change="handleSelectionChange" v-loading="productsLoading" height="400" @row-click="proRowClick" :row-class-name="proRowCName">
    ...
    <el-dialog title="修改商品" :visible.sync="updateProMask" :class="{'more-forms':moreForms}">
      <el-form :model="updateForm" :rules="proRules" label-width="100px">
        <el-form-item v-for="(item,index) in addProHead" :key="index" :label="item.label" :prop="item.prop">
          ...
          <span v-else-if="item.type=='img'">
            <span v-if="noUpdate">
              <el-upload class="upload-demo" action="" :before-upload="beforeUploadComp">
                <el-button size="small" type="primary">点击上传</el-button>
              </el-upload>
            </span>
            <span v-else>
            ...
            <span v-if="updateProIndex =='index'+scope.$index">
              ...
              <span v-else-if="item.type == 'img'">
                <span v-if="updateCompUpload=='upload'+scope.$index">
                  <el-upload action="" :before-upload="updateTableUpload">
                    <el-button size="small" type="primary">点击上传</el-button>
                  </el-upload>
                </span>
                <span v-else>
    ...
    <el-dialog title="添加Sku" :visible.sync="addSku">
      <el-form :model="skuForm" :rules="skuRules" label-width="100px">
        ...
        <el-form-item label="选择子件">
          <el-transfer filterable :filter-method="filterMethod" filter-placeholder="请输入子件编码" :titles="['子件列表', '添加列表']" v-model="skuForm.product_components" :data="compList">
          </el-transfer>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改Sku" :visible.sync="updateSkuMask">
      <el-form :model="updateSkuForm" :rules="skuRules" label-width="100px">
        ...
        <el-form-item label="选择子件">
          <el-transfer filterable :filter-method="filterMethod" filter-placeholder="请输入子件编码" :titles="['子件列表', '添加列表']" v-model="updateSkuForm.product_components" :data="updateCompList">
          </el-transfer>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    addSkuMth(row) {
      ...
      this.$fetch(this.urls.productcomponents, { is_common: true }).then(
        res => {
          res.data.map(item => {
    ...
    confirmAddSku() {
      this.$post(this.urls.combinations, this.skuForm).then(
        () => {
          this.addSku = false;
          this.$message({
            message: "添加sku成功",
    ...
    addComb() {
      ...
      if (
        this.proForm.product_components.length > 0 &&
        !this.proForm.product_components[
          this.proForm.product_components.length - 1
        ].component_code
      ) {
        this.$message({
    ...
    confirmAddPro() {
      ...
      this.$post(this.urls.products, formData).then(
        () => {
          this.$message({
            message: "添加成功",
            type: "success"
    ...
    updateAddComb() {
      ...
      if (
        this.updateForm.product_components.length > 0 &&
        !this.updateForm.product_components[
          this.updateForm.product_components.length - 1
        ].component_code
      ) {
        this.$message({
          message: "商品编码或者子件编码为空时不能添加新规格",
    ...
    confirmUpdate() {
      this.$patch(
        this.urls.products + "/" + this.updateId,
        this.updateForm
      ).then(
        () => {
    ...
    confirmUpdateSku() {
      this.$patch(
        this.urls.combinations + "/" + this.updateSkuId,
        this.updateSkuForm
      ).then(
        () => {
    ...
    confirmD(id) {
      this.$del(this.urls[this.delUrl] + "/" + id).then(
        () => {
          this.$message({
            message: "删除成功",
            type: "success"
    ...
    delMore() {
      if (this.delArr.length === 0) {
        ...
        this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$del(this.urls.products, { ids: this.delArr }).then(
              () => {
                this.$message({
                  message: "删除成功",
                  type: "success"
