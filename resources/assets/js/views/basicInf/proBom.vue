    ...
    <el-tabs>
      <el-tab-pane label="原材料" name="0">
        <el-table :data="materialData" v-loading="productsLoading">
          ...
          <el-table-column v-for="item in materialHead" :label="item.label" :width="item.width" :key="item.label" align="center">
            <template slot-scope="scope">
              <span v-if="item.type=='checkbox'">
                <el-checkbox v-model="scope.row[item.prop]" disabled></el-checkbox>
              </span>
...
export default {
  ...
  methods: {
    ...
    addConfirm() {
      ...
      this.$post(this.urls.proBom, data).then(
        () => {
    ...
    updateMaterialCName({ row, rowIndex }) {
      row.index = rowIndex;
    },
    ...
    updateMaterialDtl(row, index) {
      if (row.id) {
        this.$del(this.urls.proBomMaterial + '/' + row.id).then(() => {
    ...
    updateConfirm() {
      ...
      this.$patch(this.urls.proBom + '/' + id, data).then(
        () => {
    ...
    delPro() {
      if (this.ids.length === 0) {
        ...
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          .then(() => {
            this.$del(this.urls.proBom, { ids: this.ids }).then(
              () => {
                this.$message({
                  message: '删除成功',
                  type: 'success'
    ...
    delSingle(row, e) {
      ...
      this.delId = row.id;
      this.delUrl = row['proBomMaterial']
        ? this.urls.proBom
        : this.urls.proBomMaterial;
    },
    cancelD() {
      this.showDel = false;
      this.$message({
    ...
    /*获取数据 */
    fetchData() {
      this.$fetch(this.urls.proBom, { include: 'proBomMaterial.proBom' }).then(
        res => {
          this.productsLoading = false;
          this.productsData = res.data;
          this.materialData = res.data[0]
